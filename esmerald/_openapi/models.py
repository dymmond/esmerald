...
from openapi_schemas_pydantic.v3_1_0.open_api import OpenAPI as OpenAPI
...
from esmerald._openapi.enums import APIKeyIn, SecuritySchemeType


class APIKey(SecurityScheme):
    type: SecuritySchemeType = Field(default=SecuritySchemeType.apiKey, alias="type")
    param_in: APIKeyIn = Field(alias="in")
    name: str


class HTTPBase(SecurityScheme):
    type: SecuritySchemeType = Field(default=SecuritySchemeType.http, alias="type")
    scheme: str
...
class OAuth2(SecurityScheme):
    type: SecuritySchemeType = Field(default=SecuritySchemeType.oauth2, alias="type")
    flows: OAuthFlows


class OpenIdConnect(SecurityScheme):
    type: SecuritySchemeType = Field(default=SecuritySchemeType.openIdConnect, alias="type")
