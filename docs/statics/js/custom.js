...
async function getDataBatch(page) {
    const response = await fetch(
        `https://api.github.com/search/repositories?q=topic:esmerald&per_page=100&page=${page}`,
...
function setupTermynal() {
    ...
    function loadVisibleTermynals() {
        termynals = termynals.filter((termynal) => {
            if (
                termynal.container.getBoundingClientRect().top - innerHeight <=
                0
            ) {
                termynal.init();
                return false;
...
async function showRandomAnnouncement(groupId, timeInterval) {
    const announceEsmerald = document.getElementById(groupId);
    if (announceEsmerald) {
        let children = [].slice.call(announceEsmerald.children);
...
async function main() {
    if (div) {
        ...
        data.forEach((v) => {
            if (v.full_name === "dymmond/esmerald") {
                return;
            }
