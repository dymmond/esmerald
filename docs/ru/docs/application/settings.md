...
Использование настроек по умолчанию из `EsmeraldSettings` возможно не будет достаточно
для большинства приложений.

Поэтому требуются пользовательские настройки.

**Все пользовательские настройки должны быть унаследованы от `EsmeraldSettings`**.
...
1. Создан `AppSettings`, унаследованный от `EsmeraldSettings` с общими свойствами для всех сред.
...
Бывают случаи, когда вам просто нужно подключить существующее приложение Esmerald к другому, а это приложение
...
[EsmeraldSettings](#esmeraldapisettings-and-the-application), это означает, что каждый `settings_module`
**должен быть унаследован от EsmeraldSettings**, иначе будет выброшено исключение `ImproperlyConfigured`.
...
1. Создали объект настроек, унаследованный от основного `EsmeraldSettings передали некоторые
