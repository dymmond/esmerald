...
class SchedulerClassSettings(RavynSettings):
    enable_scheduler: bool = True


class CorsAppSettings(RavynSettings):
...
def test_cors_config():
    app = Ravyn(routes=[], settings_module=CorsAppSettings(), allow_origins=["*"])
...
def test_scheduler_class_raises_error(monkeypatch):
    ...
    with pytest.raises(ImproperlyConfigured):
        Ravyn(routes=[], settings_module=SchedulerClassSettings)
