...
def test_create_project(create_folders):
    (o, e, ss) = run_cmd("tests.cli.main:app", "ravyn createproject myproject --edgy")
    assert ss == 0
...
def test_create_project_files_with_env_var(create_folders):
    (o, e, ss) = run_cmd("tests.cli.main:app", "ravyn createproject myproject -e")
    assert ss == 0
...
def test_create_project_files_without_env_var(create_folders):
    (o, e, ss) = run_cmd(
        "tests.cli.main:app", "ravyn createproject myproject --edgy", is_app=False
    )
    assert ss == 0
