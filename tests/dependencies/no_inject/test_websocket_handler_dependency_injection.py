...
class FirstController(APIView):
    ...
    dependencies = {
        ...
        "second": controller_second_dependency,
    }

    @websocket(
        path="/{path_param:str}",
        dependencies={
            "first": local_method_first_dependency,
        },
    )
...
def test_controller_dependency_injection() -> None:
    client = create_client(
        ...
        dependencies={
            "second": router_first_dependency,
            "third": router_second_dependency,
        },
    )
...
def test_function_dependency_injection() -> None:
    @websocket(
        ...
        dependencies={
            "first": local_method_first_dependency,
            "third": local_method_second_dependency,
        },
    )
    ...
    client = create_client(
        ...
        dependencies={
            "first": router_first_dependency,
            "second": router_second_dependency,
        },
    )
    with client.websocket_connect(f"{test_path}/abcdef?query_param=12345") as ws:
        ws.send_json({"data": "123"})
